...
public class PriceRepositoryTests extends AbstractTestNGSpringContextTests {
    ...
    public void checkCurrentDatabaseContent() {
        ...
        Assert.assertEquals(priceRepository.findByProduct(PRODUCT_ID).size(), 4);
    }

    @Test
    public void checkStartAndEndDatesFromPrices() {
        final Optional<PriceEntity> secondPrice = priceRepository.findByProductAndPriceList(PRODUCT_ID, 2L);
    ...
    public void getPriceListOnFirstDesiredDates() {
        Optional<PriceEntity> price1 = priceRepository.findTopByProductAndBrandAndStartDateTimeBeforeAndEndDateTimeAfterOrderByPriorityDesc(
                PRODUCT_ID, BRAND_ID,
                LocalDateTime.of(2020, 6, 14, 10, 0),
                LocalDateTime.of(2020, 6, 14, 10, 0));
        Assert.assertTrue(price1.isPresent());
        Assert.assertEquals(price1.get().getProduct(), PRODUCT_ID);
        Assert.assertEquals(price1.get().getPriceList(), 1);

        Optional<PriceEntity> price2 = priceRepository.findTopByProductAndBrandAndStartDateTimeBeforeAndEndDateTimeAfterOrderByPriorityDesc(
                PRODUCT_ID, BRAND_ID,
                LocalDateTime.of(2020, 6, 14, 16, 0),
                LocalDateTime.of(2020, 6, 14, 16, 0));
        Assert.assertTrue(price2.isPresent());
        Assert.assertEquals(price2.get().getProduct(), PRODUCT_ID);
        Assert.assertEquals(price2.get().getPriceList(), 2);

        Optional<PriceEntity> price3 = priceRepository.findTopByProductAndBrandAndStartDateTimeBeforeAndEndDateTimeAfterOrderByPriorityDesc(
                PRODUCT_ID, BRAND_ID,
                LocalDateTime.of(2020, 6, 14, 21, 0),
                LocalDateTime.of(2020, 6, 14, 21, 0));
        Assert.assertTrue(price3.isPresent());
        Assert.assertEquals(price3.get().getProduct(), PRODUCT_ID);
        Assert.assertEquals(price3.get().getPriceList(), 1);

        Optional<PriceEntity> price4 = priceRepository.findTopByProductAndBrandAndStartDateTimeBeforeAndEndDateTimeAfterOrderByPriorityDesc(
                PRODUCT_ID, BRAND_ID,
                LocalDateTime.of(2020, 6, 15, 10, 0),
                LocalDateTime.of(2020, 6, 15, 10, 0));
        Assert.assertTrue(price4.isPresent());
        Assert.assertEquals(price4.get().getProduct(), PRODUCT_ID);
        Assert.assertEquals(price4.get().getPriceList(), 3);

        Optional<PriceEntity> price5 = priceRepository.findTopByProductAndBrandAndStartDateTimeBeforeAndEndDateTimeAfterOrderByPriorityDesc(
                PRODUCT_ID, BRAND_ID,
                LocalDateTime.of(2020, 6, 16, 21, 0),
                LocalDateTime.of(2020, 6, 16, 21, 0));
        Assert.assertTrue(price5.isPresent());
        Assert.assertEquals(price5.get().getProduct(), PRODUCT_ID);
